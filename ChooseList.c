...
INT_PTR CALLBACK ChooseCallback(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		{
			EnableWindow(GetDlgItem(hwnd, IDC_CHECK_OPTION), FALSE);
			ShowWindow(GetDlgItem(hwnd, IDC_CHECK_OPTION), SW_HIDE);
		}
...
int ChooseList(HWND hwnd, LPCTSTR title, LPCTSTR * options, int cOptions, int nDefault, LPCTSTR pcszCheck, BOOL *retChosen)
{
	...
	if (DialogBox(GetCurrentFileModule(), MAKEINTRESOURCE(IDD_DIALOG_CHOOSE), hwnd, ChooseCallback) != IDOK)
		return -1;
	if (retChosen)
